...
public class AnimeRepository {
    ...
    public static void delete(int id) {
        try (Connection conn = ConnectionFactory.getConnection();
            ...
            log.error("Error while tring to delete anime '{}'", id, e);
            //e.printStackTrace();
        }
    }
    ...
    public static void save(Anime anime) {
        log.info("Saving Anime?? ID: '{}', Name: '{}'", anime.getId(), anime.getName());
        try (Connection conn = ConnectionFactory.getConnection();
             PreparedStatement ps = createPrepareStatementSave(conn, anime)) {
            ps.execute();

        } catch (SQLException e) {
            log.error("Error while tring to update anime '{}'", anime.getId(), e);
        }
    }
